						...
						<div class="form-group">
							<select class="form-control" name="subPage" id="subPage">
								...
								<?php foreach ($blogs as $key => $blog) {
									?>
									<option <?if($id==$blog->blog_slug){?> selected <?php }?> value="<?php echo base_url() ?>admin/comments-view-blog/<?php echo $blog->blog_slug ; ?>"><?php echo $blog->blog_title; ?></option>
								<?php } ?>

							</select>
							<a href="<?php echo base_url();?>admin/list-blogs" class="btn btn-info btn-fill pull-right color-red" style="margin: 12px 3px 0;">
                                <p>Back</p>
                            </a>
						</div>
					</div>
					<div class="content">
						<div class="comments-area">
							...
							<ul class="comment-list">
								...
								<li>
									<?php
									$select = "*,subscribe.id as IDS,replys.id as replyID,subscribe.image as ImageSucribe";
										...
										<?php foreach ($replys as $key => $reply) { ?>
											...
											<li class="reply-go-<?php echo $comment->comID; ?>">
												<div class="thumb-list">
													<figure>
														<span style="background-image: url('<?php echo ($reply->ImageSucribe != null) ? base_url().'uploads/userImages/'.$reply->ImageSucribe :  base_url().'uploads/userImages/2019_12_05_06_41_17.png' ?>')"></span>
													</figure>
													<div class="text-holder">
														<h6><?php echo $reply->first_name." ".$reply->last_name ; ?></h6>
														<a href="javascript:void(0)" class="comment-reply-link delete_comment_reply newComment"  data-id="<?php echo $reply->replyID;?>" data-toggle="tooltip" title="Delete">Delete</a>
							...
							<div class="comment-respond">
								...
								<form action="javascript:void(0)" method="post" id="comment_post_blog_admin">
									...
									<input id="detect_forum" name="detect_forum" type="hidden" value="1">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="comment-respond">
					...
					<form method="post" id="coment_edit-1">
			...
			<div class="modal-body">
				<div class="comment-respond">
					...
					<form method="post" id="coment_reply">
						...
						<p class="form-submit"><input value="Reply" type="submit"></p>
					</form>
				</div>
				<div class="clearfix"></div>
			</div>
...
<script type="text/javascript">
  ...
  $('#coment_edit-1').submit(function(e) {
  e.preventDefault();
  if($(this).valid()){
  	//
  	// alert('amir ishaque');
